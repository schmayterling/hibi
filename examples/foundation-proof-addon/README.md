...
The addon suggests `[[proof-note]]` and `#proof` in source and rich editors. Type `@pr` in rich view or `[[pr` in source view for the note link. Rich wiki-link insertion works only for an exact plain-paragraph mapping; other rich ranges fall back to source view. Selecting `#proof` offers an undoable **Capitalize proof tag** action. Hovering the tag shows a short explanation.
