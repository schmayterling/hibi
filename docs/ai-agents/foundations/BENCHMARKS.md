...
- App sources: baseline `35ef3fd`; candidate `fb0a56a`. Both used their own production build and the same sampler, using Playwright `keyboard.insertText` for the numbers below. The sampler now exposes this mode as `HIBI_BENCH_INSERT_TEXT=1`; its default remains ordinary key presses. Do not read these figures as physical-key timing.
